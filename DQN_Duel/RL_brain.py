...
class DQN_Duel(object):
    ...
    def _build_net(self):
        ...
        def build_layer(s,n_ll=20,trainable=True):
            with tf.variable_scope('ll'):
                ll=tf.layers.dense(s,n_ll,tf.nn.relu,kernel_initializer=w_initializer,bias_initializer=b_initializer,name='ll',trainable=trainable)
            with tf.variable_scope('Value'):
                self.V=tf.layers.dense(ll,1,kernel_initializer=w_initializer,bias_initializer=b_initializer,trainable=trainable)
            with tf.variable_scope('Advantage'):
                self.A=tf.layers.dense(ll,self.n_actions,kernel_initializer=w_initializer,bias_initializer=b_initializer,trainable=trainable)
        ...
        with tf.variable_scope('eval_net'):
            self.q_eval=build_layer(self.s,trainable=True)

        with tf.variable_scope('target_net'):
            self.q_next=build_layer(self.s_,trainable=False)
    ...
    def learn(self):
        ...
        q_eval,q_next=self.sess.run([self.q_eval,self.q_next],{self.s:s,self.s_:s_})

        q_target=q_eval.copy()

        q_target[:,a]=r+self.gamma*np.max(q_next,axis=1)

        _,loss=self.sess.run([self.opt,self.loss],{self.s:s,self.q_target:q_target})
